...
if (getMarkerPos _x distance _positionX < distanceSPWN) then
	...
	if (count _staticsX > 0) then
		{
		_threat = _threat + ({typeOf _x in FactionGet(reb,"staticMortars")} count _staticsX) + (2*({typeOf _x in FactionGet(reb,"staticAT")} count _staticsX)) ///emmm, we should also check for other factions statics
		};
	};
