...
/* ----------------------------------------------------------------------------
Function: A3U_fnc_getParentMethod
...
Example:
    (begin example)
    [_self, "thisMethod"] call A3U_fnc_getParentMethod;
    (end example)
...
if !assert(_parent in GVAR(objectRegistry))
...
private _definition = GVAR(objectRegistry) get _parent;

if !(_methodName in _definition)
...
if ((isNil "_method") || { !(_method isEqualType {}) })
